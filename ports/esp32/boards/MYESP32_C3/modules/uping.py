...
def ping(host, count=4, timeout=5000, interval=10, quiet=False, size=64):
        ...
        while 1:
            ...
            if socks:
                ...
                if h2.type==0 and h2.id==h.id and (seq in seqs): # 0: ICMP_ECHO_REPLY
                    ...
                    delay_list.append(t_elasped) #延迟时间放入列表用于计算平均延迟
    ...
    for x in delay_list:
        delay_sum+=x
    delay_ave=delay_sum/len(delay_list)
    #返回发送数量、接收数量、丢包率、平均延时
    return (n_trans, n_recv,(n_trans-n_recv)/n_trans*100,delay_ave)